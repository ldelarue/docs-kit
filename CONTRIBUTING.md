...
force-moved to the newest release tag by `publish.yml` (commits on `main`
that are not part of a release are never exposed through it); the wheel on
the GitHub Release is byte-exact for auditable pins.
...
`shared/mise/` payload** (v0.1.x and older) - a down-pinned or version-skewed
pull then leaves the working layer untouched instead of wiping it to empty.
...
`shared/mise/` (first tag carrying `docs.toml` at all: v0.2.0). After
...
> Current repo-state mismatch: `pyproject.toml` says `0.1.0` while tags up
> to `v0.2.1` exist. A `0.1.0` CLI therefore resolves `v0.1.0` - a tag with
> no task layer - and the payload guard rejects the pull: fresh shim
> installs stay without `docs:*` tasks until the next release aligns the
> version (existing layers are protected, as above). Release a version to
> heal this.
...
3. Merge → tag `vX.Y.Z` + GitHub Release + wheel assets; `publish.yml` moves
   the `latest` branch to the tag. Consumers on `@latest` refresh via
