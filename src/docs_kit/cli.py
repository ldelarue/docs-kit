...
KIT_PYPI_FINAL = "# \"pypi:ldelarue/docs-kit\" = \"0.1.0\""
ZENSONAL_PYPI_FINAL = "# \"pypi:zensical\" = \"0.0.62\""

# Fallback used when the running kit is not an editable/source install (e.g. an
# ephemeral `uvx --from <path>` wheel from a cache dir with no .git parent).
...
# docs-kit has no git remote yet: tasks fall back to `uv run --no-dev
# --project "$DOCS_KIT"`, which rebuilds transparently when kit sources change
# (an ephemeral `uvx --from <path>` would silently serve stale cached wheels).
# The `command -v docs-kit` branch becomes active once the tool is installed
# via the pypi pins below.
# Once docs-kit is pushed, `mise use 'pypi:ldelarue/docs-kit@0.1.0'` (keep the
# key shape mise writes), uncomment the pins below, drop the [env] fallback
# and (optionally) simplify the task bodies back to bare commands:
{KIT_PYPI_FINAL}
{ZENSONAL_PYPI_FINAL}
