...
def _kit_sync_table(shim: str) -> str:
    return (
        f'\n[tasks."{KIT_SYNC_MARKER}"]\n'
        f'description = "Pin the {shim} docs task layer to the installed docs-kit version"\n'
        + "run = '''\n"
        + _KIT_SYNC_RUN.replace("{shim}", shim).lstrip("\n")
        + "'''\n"
    )
...
def _integrate_mise(root: Path, kit_home: str) -> None:
    ...
    if _is_kit_shim(kit_home):
        if KIT_SYNC_MARKER in text:
            print(f"  .mise.toml: `{KIT_SYNC_MARKER}` task already present, kept")
        else:
            text = text.rstrip("\n") + "\n" + _kit_sync_table(kit_home)
            print(f'  .mise.toml: added `[tasks."{KIT_SYNC_MARKER}"]` (shim mode)')
    _write(path, text)
...
def cmd_init(root: Path, spec_name: str, force: bool, kit_home: str) -> int:
    ...
    _integrate_gitignore(
        root,
        () if not _is_kit_shim(kit_home) else (kit_home.rstrip("/") + "/",),
    )
