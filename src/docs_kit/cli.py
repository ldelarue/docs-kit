...
KIT_GIT_FINAL = "# \"pypi:git+https://github.com/ldelarue/docs-kit.git@v0.1.1\"  # copy the exact key `mise use` writes"
ZENSONAL_PIN_FINAL = "# \"pypi:zensical\" = \"0.0.62\""
...
# Final form of docs-kit: installed by mise from its git repo, pinned to a
# tag (no PyPI publish). Follow docs-kit ROADMAP.md Phases 1-3:
#   mise use 'pypi:git+https://github.com/ldelarue/docs-kit.git@v0.1.1'
#   mise use 'pypi:zensical@0.0.62'
# paste mise's generated keys into [tools], drop the [env] DOCS_KIT line.
# Interim until then: tasks fall back to `uv run --no-dev --project "$DOCS_KIT"`
# (deliberately not `uvx --from <path>`: uv caches local-path wheels and
# silently serves stale kit code; project installs revalidate by source mtime).
# The `command -v` branches take over automatically once installed:
{KIT_GIT_FINAL}
{ZENSONAL_PIN_FINAL}

[tasks."docs:init"]
description = "Re-scaffold the docs site (refuses existing files without --force)"
run = 'if command -v docs-kit >/dev/null 2>&1; then docs-kit init; else uv run --no-dev --project "$DOCS_KIT" docs-kit init; fi'

[tasks."docs:refresh"]
description = "Re-export the OpenAPI spec and regenerate reference pages"
run = 'mise run openapi && if command -v docs-kit >/dev/null 2>&1; then docs-kit refresh; else uv run --no-dev --project "$DOCS_KIT" docs-kit refresh; fi'
...
run = 'mise run openapi && if command -v docs-kit >/dev/null 2>&1; then docs-kit check; else uv run --no-dev --project "$DOCS_KIT" docs-kit check; fi && git diff --exit-code -- docs openapi.json'
'''
    ...
    if kit_home == KIT_HOME_PLACEHOLDER:
        print(
            f"  WARNING: $DOCS_KIT is set to '{KIT_HOME_PLACEHOLDER}'. The uv-run\n"
            "  fallback will only work once you replace it with a real checkout\n"
            "  path (for a bootstrap) or remove it and install docs-kit via\n"
            "  `mise use` (for the final form). See docs-kit README/ROADMAP."
        )
