...
# docs-kit has no git remote yet: tasks fall back to `uvx --from "$DOCS_KIT"`.
# Once it is pushed, `mise use 'pypi:ldelarue/docs-kit@0.1.0'` (check the key
# shape mise writes), uncomment the pins below and drop the [env] fallback:
# {KIT_PYPI_FINAL}
# {ZENSONAL_PYPI_FINAL}

[tasks."docs:init"]
description = "Re-scaffold the docs site (refuses existing files without --force)"
run = 'if command -v docs-kit >/dev/null 2>&1; then docs-kit init; else uvx --from "$DOCS_KIT" docs-kit init; fi'

[tasks."docs:refresh"]
description = "Re-export the OpenAPI spec and regenerate reference pages"
run = 'mise run openapi && if command -v docs-kit >/dev/null 2>&1; then docs-kit refresh; else uvx --from "$DOCS_KIT" docs-kit refresh; fi'
...
run = 'mise run openapi && if command -v docs-kit >/dev/null 2>&1; then docs-kit check; else uvx --from "$DOCS_KIT" docs-kit check; fi && git diff --exit-code -- docs openapi.json'
'''
