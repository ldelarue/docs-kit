...
def strip_spec_prose(text: str) -> str:
    ...
    return "\n".join(
        l for l in text.splitlines() if not any(l.startswith(k) for k in keep)
    )
...
def test_both_specs_render_identical_pages(tmp_path):
    ...
    for rel in (cli.ENDPOINTS_PAGE, cli.API_PAGE, cli.SWAGGER_PAGE):
        assert strip_spec_prose((go / rel).read_text()) == strip_spec_prose(
            (py / rel).read_text()
        ), rel
...
def test_init_scaffolds_without_touching_mise(tmp_path, capsys):
    repo = make_repo(tmp_path, "repo", "golang.openapi.json")
    assert cli.cmd_init(repo, SPEC, False, ".docs-kit", use_mise=True) == 0
    for rel in (
        "docs/index.md",
    ...
    assert tomllib.loads(block)["task_config"]["includes"] == [
    ...
    assert (
        "copy-paste" in out
    )  # ...for manual copy, phrased as "a mise config", never as
    assert "auto-loads" not in out  # mise.local.toml heredoc wiring (old message shape)
    before = (repo / ".gitignore").read_bytes()
    assert (
        cli.cmd_init(repo, SPEC, False, ".docs-kit", use_mise=True) == 0
    )  # repair no-op
    assert before == (repo / ".gitignore").read_bytes()
    assert (
        cli.cmd_init(repo, SPEC, True, ".docs-kit", use_mise=True) == 0
    )  # --force changes nothing either
    assert before == (repo / ".gitignore").read_bytes()
    assert not (repo / ".mise.toml").exists()
...
def test_init_checkout_mode_records_absolute_kit_path(tmp_path, capsys, monkeypatch):
    ...
    assert cli.cmd_init(repo, SPEC, False, "/abs/docs-kit-checkout", use_mise=True) == 0
    capsys.readouterr()
...
def test_shared_docs_toml_ships_pull_tasks():
    ...
    assert (
        "nothing to pull" in run
    )  # checkout installs ($DOCS_KIT absolute): exit-0 guard
...
def test_pull_tasks_uses_engine_fast_path(tmp_path, monkeypatch):
    ...
    assert (shim / "shared/mise/kit-sync").read_text() == (
        KIT / "shared/mise/kit-sync"
    ).read_text()  # shadow replaced by the payload copy
    assert "dir already existed" in (shim / "stray.txt").read_text()
...
def test_pull_tasks_refuses_tag_without_payload(tmp_path, monkeypatch, setup_engine):
    ...
    if setup_engine:
        shutil.copy(
            KIT / "shared/mise/kit-sync", repo / ".docs-kit/shared/mise/kit-sync"
        )
...
def _fake_server(monkeypatch):
    ...
    monkeypatch.setattr(
        cli.subprocess, "run", lambda cmd, **kw: calls.append(cmd) or _Done()
    )
    return calls
...
def test_serve_falls_back_to_uvx_pin(tmp_path, monkeypatch):
    ...
    assert calls == [
        [
            "uvx",
            "--from",
            f"zensical=={cli.ZENSICAL_VERSION}",
            "zensical",
            "serve",
            "--dev-addr",
            "127.0.0.1:9996",
